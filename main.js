  ...
  //anticall
	conn.ws.on("CB:call", async (json) => {
		...
		let taks =  `Terdeteksi Menelpon BOT!\nSilahkan Hubungi Owner Untuk Membuka Block !\n\nNomor Owner: \n${config.owner
					...
					.join("\n")}`
		await this.sendButton(json.content[0].attrs["call-creator"], taks, wm, hwaifu.getRandom(), [['MENU', '/menu']], m, {
                mentions: this.parseMention(taks)
            })
